...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['register'])) {
        ...
        if (empty($nama) || empty($password)) {
            ...
            $stmt_check = $conn->prepare("SELECT id FROM auth WHERE nama = ?");
            ...
            if ($stmt_check->num_rows > 0) {
                ...
                $stmt = $conn->prepare("INSERT INTO auth (nama, pw) VALUES (?, ?)");
    ...
    if (isset($_POST['login'])) {
        ...
        if (empty($nama) || empty($password)) {
            ...
            $stmt = $conn->prepare("SELECT pw FROM auth WHERE nama = ?");
